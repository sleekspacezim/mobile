...
import { getLandsFavoritesHttpFunc } from "@/src/HttpServices/Queries/Properties/FavoritesHttpFuncs";
...
const FavoriteLand: React.FC<Props> = ({ setTotalproperties }) => {
  const { landFavoriteProperties, setLandFavoriteProperties } =
  ...
  const loadMoreProperties = () => {
    ...
    if (numberOfpages >= pageNumber) {
      getLandsFavoritesHttpFunc({
        ...
        .then(({ data: { properties, totalPages, count } }) => {
          setLandFavoriteProperties([...landFavoriteProperties, ...properties]);
          setNumberOfPages(totalPages);
          setTotalproperties(count);
          setPageNumber((value) => value + 1);
        })
  ...
  const text =
    "You currently do not have favorite land properties.";
  ...
  return (
    <View style={favoritePropertyTypeStyles.container}>
      ...
      {!httpError && !isLoading && landFavoriteProperties.length < 1 && (
        <View
          style={{
            flex: 1,
            paddingTop: 0,
      ...
      {!httpError &&
        !isLoading &&
        landFavoriteProperties.length > 0 &&
