...
import { getCommercialPropertyForSaleFavoritesHttpFunc } from "@/src/HttpServices/Queries/Properties/FavoritesHttpFuncs";
...
const FavoriteCommercialsForSale: React.FC<Props> = ({
  setTotalproperties,
}) => {
  const {
  ...
  const fetchProperties = () => {
    getCommercialPropertyForSaleFavoritesHttpFunc({
      page: 1,
      accessToken,
      pageLimit,
      userId: id,
  ...
  const text =
    "You currently do not have favorite commercial properties on sale.";
  ...
  return (
    <View style={favoritePropertyTypeStyles.container}>
      ...
      {!httpError &&
        !isLoading &&
        onSaleCommercialFavoriteProperties.length < 1 && (
          <View
            style={{
              flex: 1,
              paddingTop: 0,
      ...
      {!httpError &&
        !isLoading &&
        onSaleCommercialFavoriteProperties.length > 0 &&
        width <= SCREEN_BREAK_POINT && (
          <FavoritesListMobileView
            ...
            data={onSaleCommercialFavoriteProperties}
...
export default FavoriteCommercialsForSale;
