...
import { postLandHttpFunc } from "@/src/HttpServices/Mutations/Property/Land/LandHttpFuncs";
...
const Land: INoPropsReactComponent = () => {
  ...
  const [propertyGeneralDetails, setPropertyGeneralDetails] =
    useState<ILandGeneralInfo>(generalPropertyInfoIntialState);

  const [propertyFeaturesInfo, setPropertyFeaturesInfo] =
    useState<ILandFeaturesInfo>(propertyFeaturesInfoInitialState);

  const [otherPropertyInfo, setOtherPropertyInfo] = useState<ILandOtherInfo>(
    otherPropertyInfoInitialState
  );
  ...
  const handleSubmitProperty = () => {
    setIsLoading(true);
    const propertyToBeSubmitted: ILandPropertyCreation =
      createPropertyToBeSubmitted(
        propertyGeneralDetails,
  ...
  return (
    <PropertyTypeScreenWrapper>
      ...
      <MessageModal
        ...
        message={
          "Your land has been successfully submitted, potential buyers can now view it."
        }
