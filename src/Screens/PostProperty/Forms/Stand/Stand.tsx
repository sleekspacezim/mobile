...
import { postStandHttpFunc } from "@/src/HttpServices/Mutations/Property/Stands/StandsHttpFuncs";
...
const Stand: INoPropsReactComponent = () => {
  ...
  const [propertyGeneralDetails, setPropertyGeneralDetails] =
    useState<IStandGeneralInfo>(generalPropertyInfoIntialState);

  const [propertyFeaturesInfo, setPropertyFeaturesInfo] =
    useState<IStandFeaturesInfo>(propertyFeaturesInfoInitialState);

  const [otherPropertyInfo, setOtherPropertyInfo] = useState<IStandOtherInfo>(
    otherPropertyInfoInitialState
  );
  ...
  const handleSubmitProperty = () => {
    setIsLoading(true);
    const propertyToBeSubmitted: IStandPropertyCreation =
      createPropertyToBeSubmitted(
        propertyGeneralDetails,
  ...
  return (
    <PropertyTypeScreenWrapper>
      ...
      <MessageModal
        ...
        message={
          "Your stand has been successfully submitted, potential buyers can now view it."
        }
